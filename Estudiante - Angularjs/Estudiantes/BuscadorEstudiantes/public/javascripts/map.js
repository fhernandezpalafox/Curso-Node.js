...
function consultaMatricula(matricula, dialog){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            class Usuario{
                constructor(img,nombre,matricula,lat,lng) {
                    this.nombre = nombre;
            ...
            var objUsuario = new Usuario(data[0].photos[0],
                data[0].name,
                matricula,
                data[0].location.coordinates[0],
                data[0].location.coordinates[1]);

            socket.emit('agrega usuario',objUsuario);
                ...
                setInterval(function () {
                    try {
                        navigator.geolocation.getCurrentPosition(function (position) {
                            let coords = {lng: position.coords.longitude, 
                                lat: position.coords.latitude};
                            felipeMarker.setPosition(new google.maps.LatLng(coords.lat,
                                 coords.lng));

                                 //Va codigo de socket.io para actualizar su posicion y los demas 
                                 //lo vean en el mapa

                                 class UsuarioPosicion{
                                    constructor(matricula,lat,lng){
                                       this.matricula  = matricula;
                                       this.lat = lat;
                                       this.lng = lng;
                                    }
                                 ...
                                 //Cambio posicion
                                 socket.emit("cambio posicion",new UsuarioPosicion(matricula,coords.lat,coords.lng));   
...
socket.on("cambio posicion",(usuario)=>{
    console.log(usuario);

    if(sessionStorage.getItem("matricula")!=null){

        if(sessionStorage.getItem("matricula") != usuario.matricula){
